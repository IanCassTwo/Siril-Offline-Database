...
while ($i < 786432) {
    ...
    $dbh->do("
        INSERT INTO photometry (source_id, healpix8, healpix2)
        SELECT source_id, healpix8, FLOOR(source_id / 36028797018963968) AS healpix2
        FROM STARS
        WHERE has_xp_sampled is true and healpix8 = ?
