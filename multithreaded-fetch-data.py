...
def download_and_process_file(file_url):
    ...
    with gzip.open(file_path, mode='rt', encoding='utf-8') as file:
        ...
        for row in csv_reader:
            ...
            data_queue.put(f"{row['random_index']},{row['ra']},{row['dec']},{row['pmra']},{row['pmdec']},{row['phot_g_mean_mag']},{row['phot_bp_mean_mag']},{row['teff_gspphot']},{row['source_id']}\n")
...
def write_to_file():
    ...
    with open('data.csv', 'w') as file_handle:
        file_handle.write(f'random_index,ra,dec,pmra,pmdec,pmag,bmag,teff,source_id\n')
        while True:
            data = data_queue.get()
            if data is None:
